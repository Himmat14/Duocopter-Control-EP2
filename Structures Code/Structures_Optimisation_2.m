...
%% USE HERE:
design = optimize_Ibeam(0.007, 2, 2.0); % (m, degrees, non-dim)
disp(design)
...
function design = optimize_Ibeam(v_max, theta_max, SF_min)
  ...
  lb = [0.040, 0.015, 0.006, 0.006, 0.006];
  ub = [0.100, 0.060, 0.010, 0.010, 0.010];
  x0 = [0.060, 0.050, 0.008, 0.008, 0.008];
  ...
  function [c,ceq] = cons(x)
    [v_ip_tip, v_oop_tip, theta_tip, sigma_vm_max] = analyze_IB(x);
    yield = 51e6;           % Yeild stress       [Pa]
    c = [ v_ip_tip  - v_max; ...
          v_oop_tip - v_max; ...
          theta_tip - theta_max; ...
  ...
  function [v_ip_tip, v_oop_tip, theta_tip, sigma_vm_max] = analyze_IB(x)
    a0    = x(1); a_tip = x(2); t    = x(3);
    t_w   = x(4); t_f   = x(5);
    L     = 0.30;
    N   = 1e6;
    ...
    theta   = rad2deg(cumsum( T_end ./ (G .* J_sec) ) * dz);
    theta_tip = theta(end);
