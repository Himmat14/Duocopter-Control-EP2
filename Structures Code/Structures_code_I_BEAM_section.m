...
a_tip    = 1e-2;        % width @ tip  [m]
t        = 8e-3;        % constant thickness [m]
L        = 0.305;        % arm length [m]
t_f      = 60e-3;        % I Beam top thickness [m]
t_w      = 8e-3;        % I Beam midsection thickness [m]
 

P_end    = 13;          % thrust load at tip [N] (in +x)
T_end    = 0.305;       % torque at tip [N·m] (about z)
e        = 10e-3;        % eccentricity into page (y) [m]
...
%% 3) DISCRETISATION & SECTION PROPERTIES FOR I-BEAM
N   = 1e4;
...
I_ip   = (H .* B.^3) / 12 - ( (t - t_w) .* (a - 2 .* t_f).^3 ) / 12;   % bending about y-axis (in-plane)
I_oop  = (B .* H.^3) / 12 - ( (t - t_w).^3 .* (a - 2 .* t_f) ) / 12;   % bending about x-axis (out-of-plane)
